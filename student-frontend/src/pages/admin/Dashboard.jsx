...
const AdminDashboard = () => {
  ...
  const handleExport = () => {
    if (!selectedElectionId) return;
    window.open(`http://localhost:5000/api/elections/${selectedElectionId}/export?token=${localStorage.getItem('token')}`, '_blank');
  };
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {analytics.election.status === 'active' && (
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex items-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm transition shadow-sm"
                  >
                    <Square size={16} />
                    Stop Election
                  </button>
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className={`flex items-center gap-2 px-4 py-2.5 font-bold rounded-xl text-sm transition border shadow-sm ${
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 hover:bg-slate-50 font-bold rounded-xl text-sm transition shadow-sm"
                >
                  <Download size={16} />
                  Export Excel
                </button>
